...
function getJamsSearch($listmode=0, $query=null, $offset=0, $length=3, $order="date", $sort="desc")
{
	...
	if ($sort == "desc")
		$ret .= generateSearchLink($url, $query, "asc", $offset, $length, $order, "asc");
	else
		$ret .= generateSearchLink($url, $query, "desc", $offset, $length, $order, "desc");

	$ret .= generateSearchLink($url, $query, "3", $offset, "3", $order, $sort, isset($lenbold["3"]));
...
?>
	...
	<div class="footer">&copy; 2009-2024 <a href="index.php">BINK!</a> created by <a href="http://bsmith.me">Ben Smith</a>. Design by <a href="https://arcsin.se">Arcsin</a></div>
</div>
</body>
</html>
<?php
